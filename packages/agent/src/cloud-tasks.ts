...
import type { EnqueueableConfig } from './config.js';
import type { TaskName } from './tasks/registry.js';

const client = new CloudTasksClient();

// Terraform設定に基づくキューのマッピング
// TaskNameを網羅していないと型エラーになるため, タスク追加時の付け忘れを防げる
export const QUEUE_MAPPING: Record<TaskName, string> = {
  'update': 'update-queue',
...
export const enqueueTask = async(
  ...
  const parent = client.queuePath(
    config.gcpProjectId,
    config.gcpRegion,
    `${config.serviceName}-${QUEUE_MAPPING[taskName]}`
  );
  ...
  const taskId = `${safeTaskName}-${scheduledTime.getTime()}`;
  ...
  try {
    ...
    return { status: 'queued' as const, response };
  } catch (error: unknown) {
