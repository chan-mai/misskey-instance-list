...
/** 検証で不合格となった理由, excludedHostのreasonにそのまま入る */
export type InstanceRejection = {
  reason: string;
};

export type EvaluationResult = InstanceResult & {
  rejection?: InstanceRejection;
};
  ...
  if (botInfo) {
    ...
    if (botSoftware !== 'misskey' && botSoftware !== '') {
      ...
      return {
        info: null,
        error: 'UNKNOWN',
        rejection: { reason: `Not Misskey: ${botSoftware}` },
      };
    }
    ...
    if (urlsToCheck.length > 0) {
      const detectedForkUrl = urlsToCheck.find((url) =>
        FORK_PATTERNS.some((pattern) => url.includes(pattern))
      );
      if (detectedForkUrl) {
        ...
        return {
          info: null,
          error: 'UNKNOWN',
          rejection: { reason: `Fork Repository: ${detectedForkUrl}` },
        };
      }
    }
  }
...
async function resolveRepositoryInfo(repositoryUrl: string, githubToken?: string) {
          ...
          if (cached !== undefined) {
            ...
            try {
              ...
              if (ghRes.status === 403 || ghRes.status === 429) {
                console.warn(
                  `GitHub API Rate Limit Exceeded for ${repositoryName}: Status=${ghRes.status}, Remaining=${remaining}/${limit}, Reset=${reset}`
                );
              ...
              console.warn(
                `GitHub API Fetch Error for ${repositoryName}:`,
                e.message
              );
            }
          }
...
export async function saveInstance(
  ...
  if (info) {
    const repoInfo = info.repositoryUrl
