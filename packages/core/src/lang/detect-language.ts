...
// ngramsが4.4MBあり静的importすると起動CPU予算を423ms消費するため遅延ロード。
// eld自身のload()は文字列連結の動的importでngrams4種全てがバンドルされるので使わない
let eldPromise: Promise<typeof import('eld/large').eld> | undefined;
const getEld = () => (eldPromise ??= import('eld/large').then((m) => m.eld));
 ...
 */
async function detectByLibraries(text: string): Promise<string | null> {
 ...
 */
export async function detectLanguageFromTexts(
