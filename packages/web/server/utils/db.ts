...
/** createDbが受けるD1bindingの型, @cloudflare/workers-typesの版ずれを避けるため実装から導出する */
type D1Binding = Parameters<typeof createDb>[0];

/**
 * リクエストごとにD1接続を組み立てる
 *
 * D1bindingはWorkerのenvにしか存在せず, モジュールスコープのsingletonにできない。
 * cloudflare_moduleプリセットがevent.context.cloudflare.envに載せる。
 * envはwrangler types未生成だと`{}`になるため, binding型を明示する
 */
