<template>
  <div ref="rootEl" class="relative inline-flex items-center">
    ...
    <button type="button" class="group h-9 w-9 inline-flex items-center justify-center transition-colors duration-200
        ...
        focus:outline-none" :aria-label="`Theme: ${currentLabel}`" :aria-expanded="isOpen"
      @click.stop="toggle">
      
      <!-- Icons -->
      <svg v-if="current === 'light'" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z" />
      </svg>
      <svg v-else-if="current === 'dark'" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
